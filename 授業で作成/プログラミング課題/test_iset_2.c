...
int main(void) {
int a[] = { 1, 3, 5, 7 },b[]= {2,5,7},c[]={1,3};
isetp s = iset_new();
iset_addelt(s, 1); iset_addelt(s, 3); iset_addelt(s, 5); 
expect_iset(s, 9, 3, a, "initial: { 1 3 5 }"); 
isetp q = iset_new(); iset_addelt(q,2);iset_addelt(q, 7); iset_addelt(q, 5);
expect_iset(q,9,3,b,"initial: { 2 5 7 }");
isetp r = iset_dif(s, q);
expect_iset(r, 9, 2, c, "+ { 1 3 }: { 1 3 }");
