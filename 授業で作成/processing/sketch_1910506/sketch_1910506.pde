...
class Bullet{
  ...
  boolean isHit(float x,float y,float z,float check,int i,int count){
    float d = dist(x,y,z,location.x,location.y,location.z);
    if(d<25&&10*check!=direction.z){
      if(check==1&&count!=-1)return false;
      else vanish(i);return true;}
    return false;
  }
}
...
class Enemy{
  ...
  void isdamage(){
    if(bullets!=null){
      for(int i=0;i<bullets.size();i++){
        if(bullets.get(i).isHit(location.x,location.y,location.z,1,i,count))hp-=1;
        if(hp<=0&&count==-1){
...
class Player{
  ...
  void isdamage(){
    if(bullets!=null){
      for(int i=0;i<bullets.size();i++){
        if(bullets.get(i).isHit(mouseX,mouseY,0,-1,i,-1)){
          hp -=1;
        }
      }
    }
